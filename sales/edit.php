...
$res_invoice = $conn->query("SELECT * FROM sales WHERE id = $invoice_id");
$invoice = $res_invoice ? $res_invoice->fetch_assoc() : null;

if (!$invoice || $invoice['delete_status'] == 1) {
...
if (isset($_POST['btn_save'])) {
    ...
    $active_box_id = $invoice['box_id']; // الحفاظ على نفس صندوق الفاتورة الأصلي
    ...
    // البدء بالمعاملة لعكس الفاتورة السابقة وتطبيق الجديدة
    $conn->begin_transaction();
    try {
        // 1. استرجاع الكميات القديمة للمخازن وعكس مديونية العميل القديم
        $old_customer = $invoice['cust_name'];
        $res_old_items = $conn->query("SELECT * FROM sales_items WHERE sales_id = $invoice_id");
        if ($res_old_items) {
            while ($old_item = $res_old_items->fetch_assoc()) {
                $old_p_name = $old_item['name'];
                $old_qty = intval($old_item['quantity']);
                $old_remaining = doubleval($old_item['dis']); // dis يحمل المتبقي
                
                $parts = explode(' ', $old_p_name);
                $old_p_id = intval($parts[0]);
                
                if ($old_p_id > 0) {
                    $conn->query("UPDATE products SET quantity = quantity + $old_qty WHERE id = $old_p_id");
                }
                
                if (!empty($old_customer) && $old_customer !== 'عميل نقدي' && $old_remaining > 0) {
                    $conn->query("UPDATE customers SET cust_madeen = GREATEST(0, cust_madeen - $old_remaining) WHERE cust_name = '$old_customer'");
                }
            }
        }

        // 2. حذف تفاصيل الفاتورة السابقة والقيود اليومية الخاصة بها
        ...
        $sql_update = "UPDATE `sales` SET 
                        ...
                        `total` = '$total_paid_base', 
                        `prifet` = '$grand_profit_base', 
                        `remark` = '$remark', 
                        `currency_code` = '$currency_code', 
                        `exchange_rate` = '$exchange_rate', 
                        `remaining_total` = '$total_remaining_base' 
                       WHERE id = $invoice_id";
        if (!$conn->query($sql_update)) {
            throw new Exception("فشل تحديث الفاتورة الرئيسية: " . $conn->error);
        }

        // 4. إدراج بنود الفاتورة الجديدة وتعديل الكميات ومديونية العميل الجديد
        for ($i = 0; $i < $count; $i++) {
            $p_id = intval($products[$i]);
            $qty = intval($quantities[$i]);
            ...
            if ($p_id > 0 && $qty > 0) {
                $res_p = $conn->query("SELECT name FROM products WHERE id = $p_id");
                $p_row = $res_p ? $res_p->fetch_assoc() : null;
                $product_name_db = $p_row ? $conn->real_escape_string($p_row['name']) : 'منتج غير معروف';
                ...
                $sql_item = "INSERT INTO `sales_items`(`sales_id`, `id`, `cust_name`, `name`, `quantity`, `unit_price`, `bush`, `d`, `dis`, `total`, `all_tot`, `build_date`) 
                             VALUES ('$invoice_id', '$p_id', '$customer_name', '$product_field_val', '$qty', '$price_base', '$paid_base', '$disc_base', '$rem_base', '$paid_base', '$line_total_base', '$build_date')";
                $conn->query($sql_item);
                ...
                $sql_log = "INSERT INTO `inventory_log` (`product_id`, `product_name`, `type`, `qty_change`, `new_qty`, `reason`, `user`) 
                            SELECT $p_id, name, 'sale', -$qty, quantity, 'تعديل فاتورة بيع رقم #$invoice_id', '" . $_SESSION['SESS_FIRST_NAME'] . "' 
        ...
        // 5. تسجيل القيود المحاسبية للفاتورة المعدلة
        $user_display = $_SESSION['SESS_FIRST_NAME'];
        
        if ($total_paid_base > 0) {
            post_journal_entry($conn, 'sale', $invoice_id, 'الصندوق - ' . $box_name, 'المبيعات', $total_paid_base, "مبيعات نقدية (تعديل) فاتورة #$invoice_id - $customer_name", $user_display, $active_box_id, $currency_code, $exchange_rate);
        }
        
        if ($total_remaining_base > 0) {
            post_journal_entry($conn, 'sale', $invoice_id, 'الذمم المدينة - ' . $customer_name, 'المبيعات', $total_remaining_base, "مبيعات آجل (تعديل) فاتورة #$invoice_id - $customer_name", $user_display, $active_box_id, $currency_code, $exchange_rate);
        }
        
        if ($total_discount_base > 0) {
            post_journal_entry($conn, 'sale', $invoice_id, 'الخصم المسموح به (مصروف)', 'المبيعات', $total_discount_base, "خصم مبيعات (تعديل) فاتورة #$invoice_id - $customer_name", $user_display, $active_box_id, $currency_code, $exchange_rate);
        }
        
        if ($total_cost_base > 0) {
            post_journal_entry($conn, 'sale', $invoice_id, 'تكلفة البضاعة المباعة (مصروف)', 'المخزون / البضاعة', $total_cost_base, "إثبات تكلفة مبيعات (تعديل) فاتورة #$invoice_id", $user_display, $active_box_id, $currency_code, $exchange_rate);
        }

        $conn->commit();
        echo "<script>window.location='view.php?id=$invoice_id&autoprint=1';</script>";
        exit;
    } catch (Exception $e) {
        $conn->rollback();
...
// جلب المنتجات المتاحة للبحث والـ Autocomplete
$products_list = [];
...
$items_list = [];
$res_items = $conn->query("SELECT si.*, p.quantity as current_stock, p.buy_price as current_buy_price FROM sales_items si LEFT JOIN products p ON si.id = p.id WHERE si.sales_id = $invoice_id");
if ($res_items) {
    while($row = $res_items->fetch_assoc()) {
        $p_name = $row['name'];
        $parts = explode(' ', $p_name, 2);
        $clean_p_name = count($parts) > 1 ? $parts[1] : $p_name;
        
        $items_list[] = [
            'product_id' => intval($row['id']),
            'name' => $clean_p_name,
            'quantity' => intval($row['quantity']),
            'unit_price' => doubleval($row['unit_price']) / doubleval($invoice['exchange_rate']),
            'buy_price' => doubleval($row['current_buy_price'] ?? 0) / doubleval($invoice['exchange_rate']),
            'stock' => intval($row['current_stock'] ?? 0) + intval($row['quantity']), // المخزون الفعلي المتاح للفاتورة
            'paid' => doubleval($row['bush']) / doubleval($invoice['exchange_rate']),
            'discount' => doubleval($row['d']) / doubleval($invoice['exchange_rate']),
            'remaining' => doubleval($row['dis']) / doubleval($invoice['exchange_rate']),
            'line_total' => doubleval($row['all_tot']) / doubleval($invoice['exchange_rate'])
        ];
    }
}
?>
<title>تعديل فاتورة مبيعات #<?php echo $invoice_id; ?> - تكنولوجيا فون</title>
...
<div class="card-flat">
    <div class="card-header">
        <h5><?php echo get_icon('edit', 'ml-2 text-primary'); ?> تعديل فاتورة مبيعات رقم #<?php echo $invoice_id; ?></h5>
    ...
    <div class="card-body">
        <?php if (!empty($save_error)): ?>
            <div class="alert alert-danger rounded-0 mb-4">
                <strong>خطأ في تعديل الفاتورة:</strong> <?php echo htmlspecialchars($save_error); ?>
            </div>
        <?php endif; ?>
        <form method="POST" id="salesForm">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-3 col-sm-6 mb-3">
                    ...
                    <select name="customer_name" class="form-control rounded-0" required>
                        ...
                        <option value="عميل نقدي" <?php echo ($invoice['cust_name'] === 'عميل نقدي' || empty($invoice['cust_name'])) ? 'selected' : ''; ?>>عميل نقدي</option>
                        <?php
                        ...
                        if ($res_cust) {
                            while($row = $res_cust->fetch_assoc()) {
                                $sel = ($invoice['cust_name'] === $row['cust_name']) ? 'selected' : '';
                ...
                <div class="col-md-2 col-sm-6 mb-3">
                    ...
                    <select name="currency_code" id="currencySelect" class="form-control rounded-0" required>
                        <?php foreach($currencies_list as $c): ?>
                            <option value="<?php echo htmlspecialchars($c['code']); ?>" data-rate="<?php echo $c['exchange_rate']; ?>" data-symbol="<?php echo htmlspecialchars($c['symbol']); ?>" <?php echo ($invoice['currency_code'] === $c['code']) ? 'selected' : ''; ?>>
                ...
                <div class="col-md-2 col-sm-6 mb-3">
                    ...
                    <input type="number" step="any" name="exchange_rate" id="exchangeRateInput" class="form-control rounded-0 font-weight-bold text-center bg-light" value="<?php echo $invoice['exchange_rate']; ?>" readonly required>
                </div>
                ...
                <div class="col-md-3 col-sm-12 mb-3">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <span class="input-group-text bg-primary text-white rounded-0">
                        ...
                        <tr class="item-row">
                            <td>
                                <div class="product-search-container">
                                    <input type="text" class="form-control product-search-input rounded-0" placeholder="ابحث باسم المنتج أو الباركود..." value="<?php echo htmlspecialchars($item['name']); ?>" autocomplete="off" required>
                                    <input type="hidden" name="product_id[]" class="select-product" value="<?php echo $item['product_id']; ?>" data-base-sale-price="<?php echo $item['unit_price'] * $invoice['exchange_rate']; ?>" data-base-buy-price="<?php echo $item['buy_price'] * $invoice['exchange_rate']; ?>">
                                    <div class="autocomplete-dropdown d-none"></div>
                                </div>
                                <input type="hidden" name="buy_price[]" class="buy-price" value="<?php echo number_format($item['buy_price'], 2, '.', ''); ?>">
                            </td>
                            <td>
                                <input type="text" class="form-control stock-qty text-center bg-light rounded-0" readonly value="<?php echo $item['stock']; ?>">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="text" class="form-control total-input text-center bg-light rounded-0" readonly value="<?php echo number_format($item['line_total'], 2, '.', ''); ?>">
                            </td>
                            <td>
                                <input type="number" step="any" name="paid_amount[]" class="form-control paid-input text-center rounded-0" value="<?php echo number_format($item['paid'], 2, '.', ''); ?>" data-manually-edited="true">
                            </td>
                            <td>
                                <input type="number" step="any" name="discount_amount[]" class="form-control discount-input text-center rounded-0" value="<?php echo number_format($item['discount'], 2, '.', ''); ?>">
                            </td>
                            <td>
                                <input type="text" name="remaining_amount[]" class="form-control remaining-input text-center bg-light rounded-0" readonly value="<?php echo number_format($item['remaining'], 2, '.', ''); ?>">
                                <input type="hidden" class="profit-input" name="profit[]" value="<?php echo number_format(($item['unit_price'] - $item['buy_price']) * $item['quantity'] - $item['discount'], 2, '.', ''); ?>">
                            </td>
                            <td class="no-print">
            ...
            <div class="row">
                <div class="col-lg-6 mb-3">
                    ...
                    <textarea name="remark" class="form-control rounded-0" rows="3" placeholder="ملاحظات حول عملية البيع..."><?php echo htmlspecialchars($invoice['remark']); ?></textarea>
                </div>
                <div class="col-lg-6">
            ...
            <div class="mt-4 no-print text-left">
                <button type="submit" name="btn_save" class="btn-flat btn-flat-primary btn-lg px-5">
                    <?php echo get_icon('check', 'ml-1'); ?> تحديث وحفظ الفاتورة
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // إنشاء صف قالب فارغ للسطور الجديدة
    const rowTemplate = document.querySelector(".item-row").cloneNode(true);
    // تفريغه
    rowTemplate.querySelector(".product-search-input").value = "";
    rowTemplate.querySelector(".select-product").value = "";
    rowTemplate.querySelector(".select-product").removeAttribute("data-base-sale-price");
    rowTemplate.querySelector(".select-product").removeAttribute("data-base-buy-price");
    rowTemplate.querySelector(".buy-price").value = "0";
    rowTemplate.querySelector(".stock-qty").value = "0";
    rowTemplate.querySelector(".quantity-input").value = "1";
    rowTemplate.querySelector(".price-input").value = "";
    rowTemplate.querySelector(".total-input").value = "0";
    rowTemplate.querySelector(".paid-input").value = "0";
    rowTemplate.querySelector(".paid-input").removeAttribute("data-manually-edited");
    rowTemplate.querySelector(".discount-input").value = "0";
    rowTemplate.querySelector(".remaining-input").value = "0";
    rowTemplate.querySelector(".profit-input").value = "0";
    rowTemplate.querySelector(".autocomplete-dropdown").classList.add("d-none");
    rowTemplate.querySelector(".autocomplete-dropdown").innerHTML = "";
