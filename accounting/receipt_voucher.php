...
$recentVouchers = VoucherService::listVouchers($conn, ['type' => 'receipt'], 100);
...
.page-inner { padding: 4px 0px; }

/* Header Strip */
.page-title-bar {
    display: flex; align-items: center; justify-content: space-between;
    padding: 10px 0 14px; border-bottom: 2px solid #0f766e; margin-bottom: 16px;
...
.stat-box .sv { font-size: 0.9rem; font-weight: 700; color: #0f766e; }
.stat-box .sl { font-size: 0.65rem; color: #94a3b8; }
...
.form-panel-header i { color: #0f766e; }
...
.fg-field input:focus, .fg-field select:focus {
    outline: none; border-color: #0f766e;
}
.fg-field input.amount-f {
    font-size: 1.2rem; font-weight: 800; text-align: center;
    color: #065f46; border-color: #0f766e; background: #f0fdf4;
}
.fg-field input.readonly-f {
    background: #f8fafc; color: #0f766e; font-weight: 700; cursor: default;
}

/* Divider */
.sec-divider {
    display: flex; align-items: center; gap: 10px;
    margin: 4px 0 14px; font-size: 0.7rem;
...
.ac-drop {
    ...
    background: #fff; border: 1px solid #0f766e;
    max-height: 220px; overflow-y: auto; z-index: 9999;
    display: none;
}
.ac-drop.open { display: block; }
.ac-drop::-webkit-scrollbar { width: 4px; }
.ac-drop::-webkit-scrollbar-thumb { background: #0f766e; }
.ac-drop-item {
...
.ac-drop-item:hover, .ac-drop-item.sel, .ac-drop-item.active { background: #f0fdf4; color: #0f766e; font-weight: 600; }
.ac-code-tag {
...
.ac-type-tag {
    font-size: 0.65rem; padding: 1px 5px; flex-shrink: 0; font-weight: 700;
}
...
.ac-empty { padding: 12px; text-align: center; color: #94a3b8; font-size: 0.78rem; }

/* Preview */
.entry-preview {
    background: #f8fafc; border: 1px dashed #0f766e;
    padding: 10px 14px; margin-bottom: 14px;
    font-size: 0.8rem; display: none;
}
.entry-preview.show { display: block; }
.ep-title { font-size: 0.68rem; font-weight: 700; color: #0f766e; text-transform: uppercase; margin-bottom: 8px; }
.ep-dr { color: #065f46; font-weight: 700; }
.ep-cr { color: #1e40af; font-weight: 700; padding-right: 20px; }

/* Submit Row */
.submit-row { display: flex; align-items: center; gap: 10px; margin-top: 4px; }
.btn-submit-receipt {
    padding: 9px 24px; background: #0f766e; color: #fff;
    border: none; font-size: 0.83rem; font-weight: 700;
    font-family: 'Tajawal', sans-serif; cursor: pointer;
    display: flex; align-items: center; gap: 7px;
    transition: background .2s;
}
.btn-submit-receipt:hover { background: #0d6158; }
.btn-submit-receipt:disabled { background: #94a3b8; cursor: not-allowed; }
.btn-reset {
...
.filter-strip input, .filter-strip select {
    ...
    font-size: 0.78rem; font-family: 'Tajawal', sans-serif; background: #fff;
}
.filter-strip input:focus, .filter-strip select:focus {
    outline: none; border-color: #0f766e;
}
.btn-filter {
    padding: 5px 14px; background: #0f766e; color: #fff;
...
/* Badges */
.badge-posted { background: #d1fae5; color: #065f46; padding: 2px 8px; font-size: 0.7rem; font-weight: 700; }
.badge-voided { background: #fee2e2; color: #991b1b; padding: 2px 8px; font-size: 0.7rem; font-weight: 700; }
.vno { font-family: monospace; background: #eff6ff; color: #1e40af; padding: 1px 7px; font-size: 0.75rem; font-weight: 700; }
.party-name { color: #334155; font-size: 0.78rem; }
.amount-col { font-weight: 700; color: #065f46; }
.icon-btn {
    ...
    font-size: 0.75rem; color: #64748b; transition: all .15s;
}
.icon-btn:hover { background: #f0fdf4; border-color: #0f766e; color: #065f46; }
.icon-btn.del:hover { background: #fff1f2; border-color: #fca5a5; color: #9f1239; }
...
.total-chip.red  { background: #be123c; }
...
<div class="page-inner">
            ...
            <form id="rv-form" autocomplete="off">
                ...
                <div class="fg-grid c2">
                    <div class="fg-field">
                        ...
                        <input type="text" id="party_name" placeholder="مثال: شركة الأمل للتجارة..." required>
                    </div>
                    <div class="fg-field">
                        <label>البيان / الوصف</label>
                        <input type="text" id="description" placeholder="مثال: سداد رصيد فاتورة رقم...">
                    </div>
                </div>
                ...
                <div class="fg-grid c2">
                    <div class="fg-field">
                        <label>حساب الصندوق / البنك (المستلم فيه) <span class="req">*</span></label>
                        <div class="ac-wrap">
                ...
                <div class="table-responsive">
                    <table class="sys-table" id="voucher-items-table" style="background:#fff;">
                        <thead>
                            <tr>
                                <th style="width: 5%;">#</th>
                                <th style="width: 35%;">الحساب المقابل / المصدر <span class="req">*</span></th>
                ...
                <div class="mt-2 mb-4 d-flex justify-content-between align-items-center">
                    ...
                    <div style="min-width: 250px;">
                        <label class="form-label font-weight-bold text-success text-uppercase" style="font-size:0.75rem;">الإجمالي الكلي للسند</label>
                        <input type="text" id="voucher_total_display" class="amount-f form-control text-center" readonly value="0.00" style="font-size: 1.3rem; font-weight: 800; color: #065f46; border-color: #0f766e; background: #f0fdf4;">
                    </div>
                </div>

                <!-- Preview -->
                <div class="entry-preview" id="entry-preview" style="display:none; margin-bottom:14px; background:#f8fafc; border:1px dashed #0f766e; padding:10px 14px; font-size:.8rem;">
                    <div class="ep-title" style="font-size:.68rem; font-weight:700; color:#0f766e; text-transform:uppercase; margin-bottom:8px;"><i class="bi bi-eye"></i> معاينة القيد المحاسبي</div>
                ...
                <div class="submit-row" style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <button type="submit" class="btn-submit-receipt" id="submit-btn">
                    ...
                    <label style="display: inline-flex; align-items: center; gap: 6px; font-size: .8rem; font-weight: bold; color: #475569; cursor: pointer;">
                        <input type="checkbox" id="autoprint_after_save" checked style="width: 16px; height: 16px; accent-color: #0f766e;">
    ...
    <div class="records-panel">
        <div class="records-panel-header">
            <span><i class="bi bi-table"></i> سجل سندات القبض</span>
            ...
            <table class="sys-table">
                <thead>
                    <tr>
                        ...
                        <th>حساب القبض</th><th>الحساب المقابل</th>
                ...
                <tr><td colspan="10" class="no-data-row">
                    <i class="bi bi-inbox" style="font-size:1.5rem;display:block;"></i>لا توجد سندات قبض بعد
                </td></tr>
        ...
        <div class="totals-footer">
            ...
            <div class="total-chip"><i class="bi bi-arrow-down-circle"></i> مرحّل: <?= number_format($totalPostedAmt,2) ?></div>
            <?php if ($totalVoidedAmt > 0): ?>
            <div class="total-chip red"><i class="bi bi-x-circle"></i> ملغى: <?= number_format($totalVoidedAmt,2) ?></div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header" style="background:#0f172a;color:#e2e8f0;border:none;padding:10px 16px;">
                <h5 class="modal-title" style="font-size:.88rem;font-weight:700;">
                    <i class="bi bi-receipt-cutoff"></i> تفاصيل سند القبض
                </h5>
...
function switchPartyType() {
    ...
    if (currentPartyType === 'other') {
        ...
        nameInp.placeholder = 'مثال: شركة الأمل للتجارة...';
    } else {
...
function searchParty(q) {
    ...
    const src = currentPartyType==='customer' ? CUSTOMERS : SUPPLIERS;
    const ql  = q.trim().toLowerCase();
...
function addNewVoucherRow(accountId = '', accountCode = '', accountName = '', amount = '', memo = '') {
    ...
    tr.innerHTML = `
        ...
        <td>
            <input type="number" step="0.0001" min="0.01" class="row-amount amount-input form-control form-control-sm text-center font-weight-bold text-success" value="${amount || '0.00'}" oninput="updateVoucherTotal()" required style="padding: 5px 9px; font-size: .88rem; border-radius: 4px; border: 1px solid var(--border-color);">
        </td>
...
document.addEventListener('DOMContentLoaded', () => {
    addNewVoucherRow();
    setupEnterKeyNavigation('rv-form');
    switchPartyType();
});
...
function updatePreview() {
    ...
    if (total > 0 && rows.length > 0) {
        ...
        let bodyHtml = `<div><span class="ep-dr">من حـ/</span> ${cash} <strong>${total.toFixed(2)}</strong></div>`;
        rows.forEach(r => {
            bodyHtml += `<div><span class="ep-cr">إلى حـ/</span> ${r.name} <strong>${r.amount.toFixed(2)}</strong></div>`;
        });
...
/* ── Submit ── */
document.getElementById('rv-form').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    try {
        ...
        if (data.success) {
            ...
            al.innerHTML = `<i class="bi bi-check-circle-fill"></i> تم ترحيل سند القبض — رقم: <strong>${data.voucher_no}</strong>`;
...
async function viewVoucher(id) {
    ...
    document.getElementById('view-body').innerHTML = '<div class="text-center py-3"><div class="spinner-border spinner-border-sm text-success"></div></div>';
